-- Apply missing columns from migration 025 to fix due_date assignment
...
BEGIN
    -- Get task priority
    task_priority := COALESCE((task_data->>'priority')::TEXT, 'medium');
    
    -- Calculate days until goal deadline
    days_until_deadline := EXTRACT(DAY FROM (goal_record.deadline - NOW()));
    
    -- Adjust priority based on task priority level
    CASE task_priority
        WHEN 'high' THEN priority_boost := priority_boost + 20;
        WHEN 'medium' THEN priority_boost := priority_boost + 10;
        WHEN 'low' THEN priority_boost := priority_boost + 0;
    END CASE;
    
    -- Adjust priority based on urgency (days until deadline)
    IF days_until_deadline <= 1 THEN
        priority_boost := priority_boost + 30;
    ...
    IF (task_data->'dependencies') IS NOT NULL AND json_array_length(task_data->'dependencies') > 0 THEN
        priority_boost := priority_boost + 15;
    END IF;
    
    -- Adjust priority based on estimated duration (longer tasks get higher priority)
    IF COALESCE((task_data->>'estimated_duration')::INTEGER, 60) > 120 THEN
        priority_boost := priority_boost + 10;
    END IF;
